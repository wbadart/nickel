...
pub fn merge(fst: Closure, snd: Closure) -> Result<Closure, EvalError> {
    ...
    match (*t1, *t2) {
        ...
        (Term::Contract(ty), t) | (Term::ContractWithDefault(ty, _), t) => {
            ...
            let t = Term::Assume(ty, mk_merge_lbl(&p1, &p2), t).into();
            Ok(Closure { body: t, env: env2 })
        }
...
/// Generate a fresh label with an appropriate tag and without position information
/// for a cast introduced by merge
fn mk_merge_lbl(_pos1: &Option<(usize, usize)>, _pos2: &Option<(usize, usize)>) -> Label {
    Label {
